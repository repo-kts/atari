...
const AddProductionSupply = ({ onSuccess, onCancel }: { onSuccess: () => void, onCancel: () => void }) => (
    <div className="p-4">
        <p>Add Production & Supply Form Placeholder</p>
...
const EditProductionSupply = ({ item, onSuccess, onCancel }: { item: ProductionSupply, onSuccess: () => void, onCancel: () => void }) => (
    <div className="p-4">
        <p>Edit Production & Supply Form Placeholder for {item.category}</p>
        <div className="flex gap-2 mt-4">
            <Button onClick={onSuccess}>Save (Mock)</Button>
            <Button variant="outline" onClick={onCancel}>Cancel</Button>
        </div>
...
export const ProductionSupplyList: React.FC = () => {
    const { user } = useAuthStore()
    const isAdmin = user?.role === 'super_admin'
    const [items, setItems] = useState<ProductionSupply[]>([])
    const [kvks, setKvks] = useState<KVKDetails[]>([])
    const [isAddModalOpen, setIsAddModalOpen] = useState(false)
    const [editingItem, setEditingItem] = useState<ProductionSupply | null>(null)
    ...
    const loadData = () => {
        const data = localStorageService.getProductionSupplyList(isAdmin ? undefined : (user?.kvkId ?? undefined))
        setItems(data)
    }

    const handleDelete = (id: number) => {
        if (window.confirm('Are you sure you want to delete this record?')) {
            localStorageService.deleteProductionSupply(id)
            loadData()
        }
    }
    ...
    return (
        <div className="space-y-8 p-6 w-full min-w-0">
            ...
            <div className="flex flex-col gap-6">
                <h1 className="text-xl font-medium text-gray-800">Production and supply of Technological products</h1>
                        ...
                        <table className="w-full border-collapse">
                            <thead>
                                <tr className="bg-[#CFE1D1]">
                                    ...
                                    <th className="px-6 py-4 text-left text-sm font-semibold text-gray-700 tracking-wider whitespace-nowrap border-r border-white border-b border-gray-200">
                                        <div className="flex items-center gap-1.5">Category <ChevronsUpDown className="w-3 h-3 text-gray-600" /></div>
                                    </th>
                                    <th className="px-6 py-4 text-left text-sm font-semibold text-gray-700 tracking-wider whitespace-nowrap border-r border-white border-b border-gray-200">
                                        <div className="flex items-center gap-1.5">Variety <ChevronsUpDown className="w-3 h-3 text-gray-600" /></div>
                                    </th>
                                    <th className="px-6 py-4 text-left text-sm font-semibold text-gray-700 tracking-wider whitespace-nowrap border-r border-white border-b border-gray-200">
                                        <div className="flex items-center gap-1.5">Quantity <ChevronsUpDown className="w-3 h-3 text-gray-600" /></div>
                                    </th>
                                ...
                                {filteredItems.length > 0 ? (
                                    filteredItems.map((item, index) => (
                                        <tr key={item.id} className="hover:bg-[#487749]/5 transition-colors group">
                                            ...
                                            <td className="px-4 py-5 text-sm font-semibold text-gray-900">{item.category}</td>
                                            <td className="px-4 py-5 text-sm text-gray-600">{item.variety}</td>
                                            <td className="px-4 py-5 text-sm text-gray-600">{item.quantity}</td>
            ...
            <Modal
                ...
                title="Add Production & Supply Record"
                size="lg"
            >
                <AddProductionSupply
                    onSuccess={() => {
                        ...
                        const newItem: ProductionSupply = {
                            ...
                            category: 'Seeds',
                            variety: 'Wheat - HD2967',
                            quantity: 150
                        }
                        localStorageService.saveProductionSupply(newItem)
            ...
            <Modal
                ...
                title="Edit Production & Supply Record"
                size="lg"
            >
                {editingItem && (
                    <EditProductionSupply
                        item={editingItem}
                        onSuccess={() => {
                            setEditingItem(null)
                            loadData()
