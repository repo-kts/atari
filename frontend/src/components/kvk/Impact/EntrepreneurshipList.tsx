...
import { Entrepreneurship } from '../../../types/impact'
import { Modal } from '../../ui/Modal'
import { SmartBackButton } from '../../../components/common/SmartBackButton'

const AddEntrepreneurship = ({ onSuccess, onCancel }: { onSuccess: () => void, onCancel: () => void }) => (
    <div className="p-4">
        <p>Add Entrepreneurship Form Placeholder</p>
...
export const EntrepreneurshipList: React.FC = () => {
    ...
    const loadData = () => {
        const data = localStorageService.getEntrepreneurshipList(isAdmin ? undefined : (user?.kvkId ?? undefined))
        setItems(data)
    }

    const filteredItems = items.filter(item => {
        const matchesSearch =
            item.entrepreneur_name.toLowerCase().includes(searchQuery.toLowerCase()) ||
            item.enterprise_type.toLowerCase().includes(searchQuery.toLowerCase()) ||
    ...
    return (
        <div className="space-y-8 p-6 w-full min-w-0">
            ...
            <div className="flex flex-col gap-6">
                <h1 className="text-xl font-medium text-gray-800">View Details of Entrepreneurship/Startup Developed by KVK</h1>
                        ...
                        <table className="w-full border-collapse">
                            <thead>
                                <tr className="bg-[#CFE1D1]">
                                    ...
                                    <th className="px-6 py-4 text-left text-sm font-semibold text-gray-700 tracking-wider whitespace-nowrap border-r border-white border-b border-gray-200">
                                        <div className="flex items-center gap-1.5">Name of the Entrepreneur/Name of the Enterprise/Firm <ChevronsUpDown className="w-3 h-3 text-gray-600" /></div>
                                    </th>
                                    <th className="px-6 py-4 text-left text-sm font-semibold text-gray-700 tracking-wider whitespace-nowrap border-r border-white border-b border-gray-200">
                                        <div className="flex items-center gap-1.5">Type of Enterprise <ChevronsUpDown className="w-3 h-3 text-gray-600" /></div>
                                    </th>
                                    <th className="px-6 py-4 text-left text-sm font-semibold text-gray-700 tracking-wider whitespace-nowrap border-r border-white border-b border-gray-200">
                                        <div className="flex items-center gap-1.5">No of members associated <ChevronsUpDown className="w-3 h-3 text-gray-600" /></div>
                                    </th>
                                    <th className="px-6 py-4 text-left text-sm font-semibold text-gray-700 tracking-wider whitespace-nowrap border-b border-gray-200">
                                        <div className="flex items-center gap-1.5">Annual Income/Revenue of the Enterprise <ChevronsUpDown className="w-3 h-3 text-gray-600" /></div>
                                    </th>
                                </tr>
                            </thead>
                                ...
                                {filteredItems.length > 0 ? (
                                    filteredItems.map((item, index) => (
                                        <tr key={item.id} className="hover:bg-[#487749]/5 transition-colors">
                                            ...
                                            <td className="px-4 py-5 text-sm text-gray-600 font-medium">{item.entrepreneur_name}</td>
                                            <td className="px-4 py-5 text-sm text-gray-600 font-medium">{item.enterprise_type}</td>
                                            <td className="px-4 py-5 text-sm text-gray-600">{item.members_associated}</td>
                                            <td className="px-4 py-5 text-sm text-gray-600">{item.annual_income}</td>
                                        </tr>
                                    ))
                                ) : (
                                    <tr>
            ...
            <Modal
                ...
                title="Add Entrepreneurship Record"
                size="lg"
            >
                <AddEntrepreneurship
                    onSuccess={() => {
                        const newItem: Entrepreneurship = {
                            id: Date.now(),
                            kvk_id: user?.kvkId || 1,
                            kvk_name: user?.name || 'Demo KVK',
                            reporting_year: reportingYear === 'all' ? '2026' : reportingYear,
                            entrepreneur_name: 'Rahul Kumar / Green Agro',
                            enterprise_type: 'Vermi-composting',
                            members_associated: 5,
                            annual_income: '5,00,000'
                        }
                        localStorageService.saveEntrepreneurship(newItem)
