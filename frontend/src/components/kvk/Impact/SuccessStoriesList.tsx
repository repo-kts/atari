...
export const SuccessStoriesList: React.FC = () => {
    ...
    const loadData = () => {
        const data = localStorageService.getSuccessStoryList(isAdmin ? undefined : (user?.kvkId ?? undefined))
        setItems(data)
    }
    ...
    return (
        <div className="space-y-8 p-6 w-full min-w-0">
            ...
            <div className="flex flex-col gap-6">
                <h1 className="text-xl font-medium text-gray-800">View Success Stories/Case Studies</h1>
                        ...
                        <table className="w-full border-collapse">
                            <thead>
                                <tr className="bg-[#CFE1D1]">
                                    ...
                                    <th className="px-6 py-4 text-left text-sm font-semibold text-gray-700 tracking-wider whitespace-nowrap border-r border-white border-b border-gray-200">
                                        <div className="flex items-center gap-1.5">Name of the Farmer/Entrepreneur <ChevronsUpDown className="w-3 h-3 text-gray-600" /></div>
                                    </th>
                                    <th className="px-6 py-4 text-left text-sm font-semibold text-gray-700 tracking-wider whitespace-nowrap border-r border-white border-b border-gray-200">
                                        <div className="flex items-center gap-1.5">Farming Experience/Experience in Enterprise <ChevronsUpDown className="w-3 h-3 text-gray-600" /></div>
                                    </th>
                                    <th className="px-6 py-4 text-left text-sm font-semibold text-gray-700 tracking-wider whitespace-nowrap border-r border-white border-b border-gray-200">
                                        <div className="flex items-center gap-1.5">Major Achievement of the Farmers <ChevronsUpDown className="w-3 h-3 text-gray-600" /></div>
                                    </th>
                                    <th className="px-6 py-4 text-left text-sm font-semibold text-gray-700 tracking-wider whitespace-nowrap border-b border-gray-200">
                                        <div className="flex items-center gap-1.5">Title of the Success Story/Case Study <ChevronsUpDown className="w-3 h-3 text-gray-600" /></div>
                                    </th>
                                </tr>
                            </thead>
                                ...
                                {filteredItems.length > 0 ? (
                                    filteredItems.map((item, index) => (
                                        <tr key={item.id} className="hover:bg-[#487749]/5 transition-colors">
                                            ...
                                            <td className="px-4 py-5 text-sm text-gray-600 font-medium">{item.farming_experience}</td>
                                            <td className="px-4 py-5 text-sm text-gray-600">{item.major_achievement}</td>
                                            <td className="px-4 py-5 text-sm text-gray-600">{item.title}</td>
                                        </tr>
                                    ))
                                ) : (
                                    <tr>
            ...
            <Modal
                ...
                title="Add Success Story/Case Study"
                size="4xl"
            >
                <AddSuccessStory
                    onSuccess={() => {
