...
import { KVKAward } from '../../../types/awards'
import { Modal } from '../../ui/Modal'
import { SmartBackButton } from '../../../components/common/SmartBackButton'

const AddKVKAward = ({ onSuccess, onCancel }: { onSuccess: () => void, onCancel: () => void }) => (
    <div className="p-4">
        <p>Add KVK Award Form Placeholder</p>
...
export const KVKAwardList: React.FC = () => {
    const { user } = useAuthStore()
    const isAdmin = user?.role === 'super_admin'
    const [items, setItems] = useState<KVKAward[]>([])
    ...
    const loadData = () => {
        const data = localStorageService.getKVKAwardList(isAdmin ? undefined : (user?.kvkId ?? undefined))
        setItems(data)
    }
                ...
                <AddKVKAward
                    onSuccess={() => {
                        const newItem: KVKAward = {
                            ...
                            award_name: 'Best KVK',
                            amount: 100000,
                            achievement: 'Excellence in Extension',
                            conferring_authority: 'ICAR'
                        }
                        localStorageService.saveKVKAward(newItem)
