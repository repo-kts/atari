...
import { ScientistAward } from '../../../types/awards'
import { Modal } from '../../ui/Modal'
import { SmartBackButton } from '../../../components/common/SmartBackButton'

const AddScientistAward = ({ onSuccess, onCancel }: { onSuccess: () => void, onCancel: () => void }) => (
    <div className="p-4">
        <p>Add Scientist Award Form Placeholder</p>
...
export const ScientistAwardList: React.FC = () => {
    const { user } = useAuthStore()
    const isAdmin = user?.role === 'super_admin'
    const [items, setItems] = useState<ScientistAward[]>([])
    ...
    const loadData = () => {
        const data = localStorageService.getScientistAwardList(isAdmin ? undefined : (user?.kvkId ?? undefined))
        setItems(data)
    }
    ...
    const filteredItems = items.filter(item => {
        const matchesSearch =
            ...
            item.scientist_name.toLowerCase().includes(searchQuery.toLowerCase()) ||
    ...
    return (
        <div className="space-y-8 p-6 w-full min-w-0">
                        ...
                        <table className="w-full border-collapse">
                            <thead>
                                <tr className="bg-[#D1E1D1] border-b border-[#487749]/10">
                                    ...
                                    <th className="px-6 py-4 text-left text-sm font-bold text-[#374151] tracking-wider whitespace-nowrap">
                                        <div className="flex items-center gap-1.5">Head/Scientist <ChevronsUpDown className="w-3 h-3 text-gray-400" /></div>
                                    </th>
                                    <th className="px-6 py-4 text-left text-sm font-bold text-[#374151] tracking-wider whitespace-nowrap">
                                ...
                                {filteredItems.length > 0 ? (
                                    filteredItems.map((item, index) => (
                                        <tr key={item.id} className="hover:bg-[#487749]/5 transition-colors">
                                            ...
                                            <td className="px-4 py-5 text-sm text-gray-600 font-medium">{item.scientist_name}</td>
            ...
            <Modal
                ...
                title="Add Scientist Award"
                size="lg"
            >
                <AddScientistAward
                    onSuccess={() => {
                        const newItem: ScientistAward = {
                            id: Date.now(),
                            kvk_id: user?.kvkId || 1,
                            ...
                            conferring_authority: 'ICAR'
                        }
                        localStorageService.saveScientistAward(newItem)
