...
import { KisanSarathiRecord } from '../../../types/miscellaneous'
import { SmartBackButton } from '../../../components/common/SmartBackButton'

export const KisanSarathiList: React.FC = () => {
    const { user } = useAuthStore()
    const isAdmin = user?.role === 'super_admin'
    const [items, setItems] = useState<KisanSarathiRecord[]>([])
    const [searchQuery, setSearchQuery] = useState('')
    const [reportingYear, setReportingYear] = useState('2026')
    const [kvkFilter, setKvkFilter] = useState('All')

    useEffect(() => {
        // Mock data from image 4
        setItems([
            { id: 1, kvk_name: 'KVK Madhepura', registered_farmers_count: '66484', phone_calls_addressed: '357', answered_calls: '294', reporting_year: '2026' },
            { id: 2, kvk_name: 'KVK Lohardaga', registered_farmers_count: '5247', phone_calls_addressed: '2000', answered_calls: '1050', reporting_year: '2026' },
        ])
    }, [user, isAdmin])
    ...
    return (
        <div className="space-y-8 p-6 w-full min-w-0 font-sans">
            ...
            <div className="flex flex-col gap-6">
                <h1 className="text-xl font-medium text-gray-800">View Details of Kisan Sarathi</h1>
                ...
                <div className="flex flex-wrap items-end gap-4">
                    ...
                    <div className="flex flex-col gap-2">
                        ...
                        >
                            <option value="All">All</option>
                            <option value="KVK Madhepura">KVK Madhepura</option>
                            <option value="KVK Lohardaga">KVK Lohardaga</option>
                        </select>
                    </div>
                ...
                <CardContent className="p-0">
                        ...
                        <table className="w-full border-collapse">
                            <thead>
                                <tr className="bg-[#CFE1D1]">
                                    ...
                                    <th className="px-4 py-3 text-left text-sm font-semibold text-gray-700 tracking-wider whitespace-nowrap border-r border-white border-b border-gray-200">
                                        <div className="flex items-center justify-between">No. of farmers registered on KSP portal <ChevronsUpDown className="w-4 h-4 text-gray-600" /></div>
                                    </th>
                                    <th className="px-4 py-3 text-left text-sm font-semibold text-gray-700 tracking-wider whitespace-nowrap border-r border-white border-b border-gray-200">
                                        <div className="flex items-center justify-between">Phone call addressed <ChevronsUpDown className="w-4 h-4 text-gray-600" /></div>
                                    </th>
                                    <th className="px-4 py-3 text-left text-sm font-semibold text-gray-700 tracking-wider whitespace-nowrap border-b border-gray-200">
                                        <div className="flex items-center justify-between">Aswered call <ChevronsUpDown className="w-4 h-4 text-gray-600" /></div>
                                    </th>
                                </tr>
                            </thead>
                                ...
                                {filteredItems.length > 0 ? (
                                    filteredItems.map((item, index) => (
                                        <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                                            ...
                                            <td className="px-4 py-3 text-sm text-gray-700 border-r border-gray-200">{item.registered_farmers_count}</td>
                                            <td className="px-4 py-3 text-sm text-gray-700 border-r border-gray-200">{item.phone_calls_addressed}</td>
                                            <td className="px-4 py-3 text-sm text-gray-700">{item.answered_calls}</td>
                                        </tr>
                                    ))
                                ) : (
                                    <tr>
                                        <td colSpan={5} className="px-4 py-12 text-center border-t border-gray-200">
                    ...
                    <div className="px-4 py-4 border-t border-gray-200 flex items-center justify-between">
                        ...
                        <div className="flex gap-1">
                            <Button variant="outline" size="sm" className="px-3" disabled={filteredItems.length <= 10}>Previous</Button>
                            <Button variant="primary" size="sm" className="px-3 bg-[#487749]">1</Button>
                            <Button variant="outline" size="sm" className="px-3" disabled={filteredItems.length <= 10}>Next</Button>
                        </div>
                    </div>
                </CardContent>
            </Card>
