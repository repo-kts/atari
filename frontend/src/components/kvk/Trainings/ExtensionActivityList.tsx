...
import { AddExtensionActivity } from './AddExtensionActivity'
import { EditExtensionActivity } from './EditExtensionActivity'
import { SmartBackButton } from '../../../components/common/SmartBackButton'
import { KVKDetails } from '../../../types/kvk'

export const ExtensionActivityList: React.FC = () => {
    ...
    const loadData = () => {
        const data = localStorageService.getExtensionActivityList(isAdmin ? undefined : (user?.kvkId ?? undefined))
        setActivities(data)
    }

    const handleDelete = (id: number) => {
        if (window.confirm('Are you sure you want to delete this extension activity record?')) {
            localStorageService.deleteExtensionActivity(id)
            loadData()
        }
    }
    ...
    return (
        <div className="space-y-8 p-6 w-full min-w-0">
            ...
            <div className="flex flex-col gap-6">
                <h1 className="text-xl font-medium text-gray-800">View Extension Activity</h1>
                        ...
                        <table className="w-full border-collapse">
                            <thead>
                                <tr className="bg-[#CFE1D1]">
                                    ...
                                    <th className="px-6 py-4 text-left text-sm font-semibold text-gray-700 tracking-wider whitespace-nowrap border-r border-white border-b border-gray-200">
                                        <div className="flex items-center gap-1.5">No. of participants <ChevronsUpDown className="w-3 h-3 text-gray-600" /></div>
                                    </th>
                            ...
                            <tbody className="divide-y divide-[#487749]/5">
                                {filteredActivities.length > 0 ? (
                                    ...
                                    <tr>
                                        <td colSpan={isAdmin ? 9 : 8} className="px-4 py-20 text-center">
                                            <div className="flex flex-col items-center gap-3">
                                                <div className="w-16 h-16 bg-gray-50 rounded-full flex items-center justify-center">
                                                    <Sprout className="w-8 h-8 text-gray-300" />
                                                </div>
                                                <p className="text-gray-500 font-medium text-lg">No extension records found</p>
            ...
            <Modal
                ...
                title="Edit Extension Activity"
                size="lg"
            >
                {editingActivity && (
                    <EditExtensionActivity
