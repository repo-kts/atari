...
import { AddTechnologyWeek } from './AddTechnologyWeek'
import { EditTechnologyWeek } from './EditTechnologyWeek'
import { SmartBackButton } from '../../../components/common/SmartBackButton'
import { KVKDetails } from '../../../types/kvk'

export const TechnologyWeekList: React.FC = () => {
    const { user } = useAuthStore()
    const isAdmin = user?.role === 'super_admin'
    const [activities, setActivities] = useState<TechnologyWeek[]>([])
    const [kvks, setKvks] = useState<KVKDetails[]>([])
    const [isAddModalOpen, setIsAddModalOpen] = useState(false)
    const [editingActivity, setEditingActivity] = useState<TechnologyWeek | null>(null)
    ...
    const loadData = () => {
        const data = localStorageService.getTechnologyWeekList(isAdmin ? undefined : (user?.kvkId ?? undefined))
        setActivities(data)
    }

    const handleDelete = (id: number) => {
        if (window.confirm('Are you sure you want to delete this technology week celebration record?')) {
            localStorageService.deleteTechnologyWeek(id)
            loadData()
        }
    }
    ...
    const filteredActivities = activities.filter(item => {
        const matchesSearch =
            item.type_of_activity.toLowerCase().includes(searchQuery.toLowerCase()) ||
            item.kvk_name.toLowerCase().includes(searchQuery.toLowerCase()) ||
            item.related_technology.toLowerCase().includes(searchQuery.toLowerCase())
    ...
    return (
        <div className="space-y-8 p-6 w-full min-w-0">
            ...
            <div className="flex flex-col gap-6">
                <h1 className="text-xl font-medium text-gray-800">View Technology Week Celebration</h1>
                        ...
                        <table className="w-full border-collapse">
                            <thead>
                                <tr className="bg-[#CFE1D1]">
                                    ...
                                    <th className="px-6 py-4 text-left text-sm font-semibold text-gray-700 tracking-wider whitespace-nowrap border-r border-white border-b border-gray-200">
                                        <div className="flex items-center gap-1.5">KVK <ChevronsUpDown className="w-3 h-3 text-gray-600" /></div>
                                    </th>
                                    <th className="px-6 py-4 text-left text-sm font-semibold text-gray-700 tracking-wider whitespace-nowrap border-r border-white border-b border-gray-200">
                                        <div className="flex items-center gap-1.5">Type of activities <ChevronsUpDown className="w-3 h-3 text-gray-600" /></div>
                                    </th>
                                    <th className="px-6 py-4 text-left text-sm font-semibold text-gray-700 tracking-wider whitespace-nowrap border-r border-white border-b border-gray-200">
                                        <div className="flex items-center gap-1.5">No. of activities <ChevronsUpDown className="w-3 h-3 text-gray-600" /></div>
                                    </th>
                                    <th className="px-6 py-4 text-left text-sm font-semibold text-gray-700 tracking-wider whitespace-nowrap border-r border-white border-b border-gray-200">
                                        <div className="flex items-center gap-1.5">Related crop/livestock technology <ChevronsUpDown className="w-3 h-3 text-gray-600" /></div>
                                    </th>
                                    <th className="px-6 py-4 text-left text-sm font-semibold text-gray-700 tracking-wider whitespace-nowrap border-r border-white border-b border-gray-200">
                                        <div className="flex items-center gap-1.5">Number of participants <ChevronsUpDown className="w-3 h-3 text-gray-600" /></div>
                                    </th>
                                ...
                                {filteredActivities.length > 0 ? (
                                    filteredActivities.map((item, index) => (
                                        <tr key={item.id} className="hover:bg-[#487749]/5 transition-colors group">
                                            ...
                                            <td className="px-4 py-5 text-sm font-semibold text-gray-900">{item.type_of_activity}</td>
                                            <td className="px-4 py-5 text-sm text-gray-600">{item.no_of_activities}</td>
                                            <td className="px-4 py-5 text-sm text-gray-600">{item.related_technology}</td>
                                            <td className="px-4 py-5 text-sm text-gray-600">{item.no_of_participants}</td>
                                    ...
                                    <tr>
                                        <td colSpan={9} className="px-4 py-20 text-center">
                                            <div className="flex flex-col items-center gap-3">
                                                ...
                                                <p className="text-gray-500 font-medium text-lg">No technology week records found</p>
