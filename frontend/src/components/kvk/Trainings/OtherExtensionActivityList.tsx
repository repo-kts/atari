...
import { AddOtherExtensionActivity } from './AddOtherExtensionActivity'
import { EditOtherExtensionActivity } from './EditOtherExtensionActivity'
import { SmartBackButton } from '../../../components/common/SmartBackButton'
import { KVKDetails } from '../../../types/kvk'

export const OtherExtensionActivityList: React.FC = () => {
    const { user } = useAuthStore()
    const isAdmin = user?.role === 'super_admin'
    const [activities, setActivities] = useState<OtherExtensionActivity[]>([])
    const [kvks, setKvks] = useState<KVKDetails[]>([])
    const [isAddModalOpen, setIsAddModalOpen] = useState(false)
    const [editingActivity, setEditingActivity] = useState<OtherExtensionActivity | null>(null)
    ...
    const loadData = () => {
        const data = localStorageService.getOtherExtensionActivityList(isAdmin ? undefined : (user?.kvkId ?? undefined))
        setActivities(data)
    }

    const handleDelete = (id: number) => {
        if (window.confirm('Are you sure you want to delete this other extension activity record?')) {
            localStorageService.deleteOtherExtensionActivity(id)
            loadData()
        }
    }
    ...
    const filteredActivities = activities.filter(item => {
        const matchesSearch =
            item.nature_of_activity.toLowerCase().includes(searchQuery.toLowerCase()) ||
    ...
    return (
        <div className="space-y-8 p-6 w-full min-w-0">
            ...
            <div className="flex flex-col gap-6">
                <h1 className="text-xl font-medium text-gray-800">Other Extension Activity</h1>
                        ...
                        <table className="w-full border-collapse">
                            <thead>
                                <tr className="bg-[#CFE1D1]">
                                    ...
                                    <th className="px-6 py-4 text-left text-sm font-semibold text-gray-700 tracking-wider whitespace-nowrap border-r border-white border-b border-gray-200">
                                        <div className="flex items-center gap-1.5">Nature of Extension Activity <ChevronsUpDown className="w-3 h-3 text-gray-600" /></div>
                                    </th>
                                    <th className="px-6 py-4 text-left text-sm font-semibold text-gray-700 tracking-wider whitespace-nowrap border-r border-white border-b border-gray-200">
                                ...
                                {filteredActivities.length > 0 ? (
                                    filteredActivities.map((item, index) => (
                                        <tr key={item.id} className="hover:bg-[#487749]/5 transition-colors group">
                                            ...
                                            <td className="px-4 py-5 text-sm font-semibold text-gray-900">{item.nature_of_activity}</td>
                                    ...
                                    <tr>
                                        <td colSpan={isAdmin ? 6 : 5} className="px-4 py-20 text-center">
                                            <div className="flex flex-col items-center gap-3">
                                                <div className="w-16 h-16 bg-gray-50 rounded-full flex items-center justify-center">
                                                    <Sprout className="w-8 h-8 text-gray-300" />
                                                </div>
                                                <p className="text-gray-500 font-medium text-lg">No other extension records found</p>
            ...
            <Modal
                ...
                title="Add Other Extension Activity"
                size="lg"
            >
                <AddOtherExtensionActivity
                    onSuccess={() => {
            ...
            <Modal
                ...
                title="Edit Other Extension Activity"
                size="lg"
            >
                {editingActivity && (
                    <EditOtherExtensionActivity
