...
import { SpecialProgram } from '../../../types/linkages'
import { Modal } from '../../ui/Modal'
import { SmartBackButton } from '../../../components/common/SmartBackButton'

const AddSpecialProgram = ({ onSuccess, onCancel }: { onSuccess: () => void, onCancel: () => void }) => (
    <div className="p-4">
        <p>Add Special Program Form Placeholder</p>
...
export const SpecialProgramList: React.FC = () => {
    const { user } = useAuthStore()
    const isAdmin = user?.role === 'super_admin'
    const [items, setItems] = useState<SpecialProgram[]>([])
    ...
    const loadData = () => {
        const data = localStorageService.getSpecialProgramList(isAdmin ? undefined : (user?.kvkId ?? undefined))
        setItems(data)
    }
    ...
    return (
        <div className="space-y-8 p-6 w-full min-w-0">
            ...
            <div className="flex flex-col gap-6">
                <h1 className="text-xl font-medium text-gray-800">View List of Special Programmes Undertaken by the KVK</h1>
                        ...
                        <table className="w-full border-collapse">
                            <thead>
                                <tr className="bg-[#CFE1D1]">
                                    ...
                                    <th className="px-6 py-4 text-left text-sm font-semibold text-gray-700 tracking-wider whitespace-nowrap border-r border-white border-b border-gray-200">
                                        <div className="flex items-center gap-1.5">Programme Type <ChevronsUpDown className="w-3 h-3 text-gray-600" /></div>
                                    </th>
                                    <th className="px-6 py-4 text-left text-sm font-semibold text-gray-700 tracking-wider whitespace-nowrap border-r border-white border-b border-gray-200">
                                        <div className="flex items-center gap-1.5">Name of the Programme/Scheme <ChevronsUpDown className="w-3 h-3 text-gray-600" /></div>
                                    </th>
                                    <th className="px-6 py-4 text-left text-sm font-semibold text-gray-700 tracking-wider whitespace-nowrap border-b border-gray-200">
                                        <div className="flex items-center gap-1.5">Date/Month of initiation <ChevronsUpDown className="w-3 h-3 text-gray-600" /></div>
                                    </th>
                                </tr>
                            </thead>
                                ...
                                {filteredItems.length > 0 ? (
                                    filteredItems.map((item, index) => (
                                        <tr key={item.id} className="hover:bg-[#487749]/5 transition-colors">
                                            ...
                                            <td className="px-4 py-5 text-sm text-gray-600 font-medium">{item.program_type}</td>
                                            <td className="px-4 py-5 text-sm text-gray-600 font-medium">{item.program_name}</td>
                                            <td className="px-4 py-5 text-sm text-gray-600">{item.initiation_date}</td>
                                        </tr>
                                    ))
                                ) : (
                                    <tr>
            ...
            <Modal
                ...
                title="Add Special Program"
                size="lg"
            >
                <AddSpecialProgram
                    onSuccess={() => {
                        const newItem: SpecialProgram = {
                            id: Date.now(),
                            kvk_id: user?.kvkId || 1,
                            kvk_name: user?.name || 'Demo KVK',
                            reporting_year: reportingYear === 'all' ? '2026' : reportingYear,
                            program_type: 'Government Scheme',
                            program_name: 'Swachh Bharat Abhiyan',
                            initiation_date: 'Oct 2024'
                        }
                        localStorageService.saveSpecialProgram(newItem)
