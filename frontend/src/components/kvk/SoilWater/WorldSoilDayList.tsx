...
import { WorldSoilDay } from '../../../types/soilWater'
import { Modal } from '../../ui/Modal'
import { SmartBackButton } from '../../../components/common/SmartBackButton'

const AddWorldSoilDay = ({ onSuccess, onCancel }: { onSuccess: () => void, onCancel: () => void }) => (
    <div className="p-4">
        <p>Add World Soil Day Celebration Form Placeholder</p>
...
export const WorldSoilDayList: React.FC = () => {
    const { user } = useAuthStore()
    const isAdmin = user?.role === 'super_admin'
    const [items, setItems] = useState<WorldSoilDay[]>([])
    ...
    const loadData = () => {
        const data = localStorageService.getWorldSoilDayList(isAdmin ? undefined : (user?.kvkId ?? undefined))
        setItems(data)
    }

    const filteredItems = items.filter(item => {
        const matchesSearch =
            item.vip_names.toLowerCase().includes(searchQuery.toLowerCase()) ||
    ...
    return (
        <div className="space-y-8 p-6 w-full min-w-0">
            ...
            <div className="flex flex-col gap-6">
                <h1 className="text-2xl font-bold text-[#212121]">Details of World Soil Day Celebration</h1>
                        ...
                        <table className="w-full border-collapse">
                            <thead>
                                <tr className="bg-[#D1E1D1] border-b border-[#487749]/10">
                                    ...
                                    <th className="px-6 py-4 text-left text-sm font-bold text-[#374151] tracking-wider whitespace-nowrap">
                                        <div className="flex items-center gap-1.5">No. of Activity conducted <ChevronsUpDown className="w-3 h-3 text-gray-400" /></div>
                                    </th>
                                    <th className="px-6 py-4 text-left text-sm font-bold text-[#374151] tracking-wider whitespace-nowrap">
                                        <div className="flex items-center gap-1.5">Soil Health Cards distributed <ChevronsUpDown className="w-3 h-3 text-gray-400" /></div>
                                    </th>
                                    <th className="px-6 py-4 text-left text-sm font-bold text-[#374151] tracking-wider whitespace-nowrap">
                                        <div className="flex items-center gap-1.5">No of VIP <ChevronsUpDown className="w-3 h-3 text-gray-400" /></div>
                                    </th>
                                    <th className="px-6 py-4 text-left text-sm font-bold text-[#374151] tracking-wider whitespace-nowrap">
                                        <div className="flex items-center gap-1.5">Name (s) of VIP(s) involved if any <ChevronsUpDown className="w-3 h-3 text-gray-400" /></div>
                                    </th>
                                    <th className="px-6 py-4 text-left text-sm font-bold text-[#374151] tracking-wider whitespace-nowrap">
                                        <div className="flex items-center gap-1.5">Total No. of Participants attended the program <ChevronsUpDown className="w-3 h-3 text-gray-400" /></div>
                                    </th>
                                </tr>
                            </thead>
                                ...
                                {filteredItems.length > 0 ? (
                                    filteredItems.map((item, index) => (
                                        <tr key={item.id} className="hover:bg-[#487749]/5 transition-colors">
                                            ...
                                            <td className="px-4 py-5 text-sm text-gray-600 font-medium">{item.activities_conducted}</td>
                                            <td className="px-4 py-5 text-sm text-gray-600">{item.soil_health_cards_distributed}</td>
                                            <td className="px-4 py-5 text-sm text-gray-600">{item.vip_count}</td>
                                            <td className="px-4 py-5 text-sm text-gray-600">{item.vip_names}</td>
                                            <td className="px-4 py-5 text-sm text-gray-600">{item.participants_count}</td>
                                        </tr>
                                    ))
                                ) : (
                                    <tr>
            ...
            <Modal
                ...
                title="Add World Soil Day Celebration"
                size="lg"
            >
                <AddWorldSoilDay
                    onSuccess={() => {
                        const newItem: WorldSoilDay = {
                            id: Date.now(),
                            kvk_id: user?.kvkId || 1,
                            kvk_name: user?.name || 'Demo KVK',
                            reporting_year: reportingYear === 'all' ? '2026' : reportingYear,
                            activities_conducted: 2,
                            soil_health_cards_distributed: 100,
                            vip_count: 1,
                            vip_names: 'District Collector',
                            participants_count: 200
                        }
                        localStorageService.saveWorldSoilDay(newItem)
