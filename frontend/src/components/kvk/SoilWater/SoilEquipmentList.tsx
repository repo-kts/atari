...
import { SoilEquipment } from '../../../types/soilWater'
import { Modal } from '../../ui/Modal'
import { SmartBackButton } from '../../../components/common/SmartBackButton'

// Placeholder Add Component
const AddSoilEquipment = ({ onSuccess, onCancel }: { onSuccess: () => void, onCancel: () => void }) => (
    <div className="p-4">
        <p>Add Equipment Form Placeholder</p>
...
export const SoilEquipmentList: React.FC = () => {
    const { user } = useAuthStore()
    const isAdmin = user?.role === 'super_admin'
    const [items, setItems] = useState<SoilEquipment[]>([])
    ...
    const loadData = () => {
        const data = localStorageService.getSoilEquipmentList(isAdmin ? undefined : (user?.kvkId ?? undefined))
        setItems(data)
    }
    ...
    return (
        <div className="space-y-8 p-6 w-full min-w-0">
            ...
            <div className="flex flex-col gap-6">
                <h1 className="text-2xl font-bold text-[#212121]">Soil and Water Testing</h1>
                        ...
                        <table className="w-full border-collapse">
                            <thead>
                                <tr className="bg-[#D1E1D1] border-b border-[#487749]/10">
                                    ...
                                    <th className="px-6 py-4 text-left text-sm font-bold text-[#374151] tracking-wider whitespace-nowrap">
                                        <div className="flex items-center gap-1.5">Equipment Name <ChevronsUpDown className="w-3 h-3 text-gray-400" /></div>
                                    </th>
                                    <th className="px-6 py-4 text-left text-sm font-bold text-[#374151] tracking-wider whitespace-nowrap">
                                        <div className="flex items-center gap-1.5">Quantity <ChevronsUpDown className="w-3 h-3 text-gray-400" /></div>
                                    </th>
                                </tr>
                            </thead>
                                ...
                                {filteredItems.length > 0 ? (
                                    filteredItems.map((item, index) => (
                                        <tr key={item.id} className="hover:bg-[#487749]/5 transition-colors">
                                            ...
                                            <td className="px-4 py-5 text-sm text-gray-600">{item.analysis}</td>
                                            <td className="px-4 py-5 text-sm text-gray-600">{item.equipment_name}</td>
            ...
            >
                <AddSoilEquipment
                    onSuccess={() => {
                        ...
                        localStorageService.saveSoilEquipment(newItem)
