...
import { RevenueGeneration } from '../../../types/financialPerformance'
import { SmartBackButton } from '../../../components/common/SmartBackButton'

export const RevenueGenerationList: React.FC = () => {
    const { user } = useAuthStore()
    const isAdmin = user?.role === 'super_admin'
    const [items, setItems] = useState<RevenueGeneration[]>([])
    ...
    const filteredItems = items.filter(item => {
        const matchesSearch =
            ...
            item.name_of_head.toLowerCase().includes(searchQuery.toLowerCase()) ||
            item.sponsoring_agency.toLowerCase().includes(searchQuery.toLowerCase())
    ...
    return (
        <div className="space-y-8 p-6 w-full min-w-0">
            ...
            <div className="flex flex-col gap-6">
                <h1 className="text-xl font-medium text-gray-800">View Revenue Generation</h1>
                        ...
                        <table className="w-full border-collapse">
                            <thead>
                                <tr className="bg-[#CFE1D1]">
                                    ...
                                    <th className="px-4 py-3 text-left text-sm font-semibold text-gray-700 tracking-wider whitespace-nowrap border-r border-white border-b border-gray-200">
                                        <div className="flex items-center justify-between">Name of Head <ChevronsUpDown className="w-4 h-4 text-gray-600" /></div>
                                    </th>
                                    <th className="px-4 py-3 text-left text-sm font-semibold text-gray-700 tracking-wider whitespace-nowrap border-r border-white border-b border-gray-200">
                                        <div className="flex items-center justify-between">Income (Rs.) <ChevronsUpDown className="w-4 h-4 text-gray-600" /></div>
                                    </th>
                                    <th className="px-4 py-3 text-left text-sm font-semibold text-gray-700 tracking-wider whitespace-nowrap border-b border-gray-200">
                                        <div className="flex items-center justify-between">Sponsoring agency <ChevronsUpDown className="w-4 h-4 text-gray-600" /></div>
                                    </th>
                                </tr>
                            </thead>
                                ...
                                {filteredItems.length > 0 ? (
                                    filteredItems.map((item, index) => (
                                        <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                                            ...
                                            <td className="px-4 py-3 text-sm text-gray-700 border-r border-gray-200">{item.name_of_head}</td>
                                            <td className="px-4 py-3 text-sm text-gray-700 border-r border-gray-200">{item.income_rs}</td>
                                            <td className="px-4 py-3 text-sm text-gray-700">{item.sponsoring_agency}</td>
                                        </tr>
                                    ))
                                ) : (
                                    <tr>
