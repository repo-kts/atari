...
import { BudgetDetail } from '../../../types/financialPerformance'
import { SmartBackButton } from '../../../components/common/SmartBackButton'

export const BudgetDetailsList: React.FC = () => {
    const { user } = useAuthStore()
    const isAdmin = user?.role === 'super_admin'
    const [items, setItems] = useState<BudgetDetail[]>([])
    ...
    return (
        <div className="space-y-8 p-6 w-full min-w-0">
            ...
            <div className="flex flex-col gap-6">
                <h1 className="text-xl font-medium text-gray-800">View Budget Details</h1>
                        ...
                        <table className="w-full border-collapse">
                            <thead>
                                <tr className="bg-[#CFE1D1]">
                                    ...
                                    <th className="px-4 py-3 text-left text-sm font-semibold text-gray-700 tracking-wider whitespace-nowrap border-r border-white border-b border-gray-200">
                                        <div className="flex items-center justify-between">Salary Allocation <ChevronsUpDown className="w-4 h-4 text-gray-600" /></div>
                                    </th>
                                    <th className="px-4 py-3 text-left text-sm font-semibold text-gray-700 tracking-wider whitespace-nowrap border-r border-white border-b border-gray-200">
                                        <div className="flex items-center justify-between">Salary Expenditure <ChevronsUpDown className="w-4 h-4 text-gray-600" /></div>
                                    </th>
                                    <th className="px-4 py-3 text-left text-sm font-semibold text-gray-700 tracking-wider whitespace-nowrap border-r border-white border-b border-gray-200">
                                        <div className="flex items-center justify-between">General Main Grant Allocation <ChevronsUpDown className="w-4 h-4 text-gray-600" /></div>
                                    </th>
                                    <th className="px-4 py-3 text-left text-sm font-semibold text-gray-700 tracking-wider whitespace-nowrap border-r border-white border-b border-gray-200">
                                        <div className="flex items-center justify-between">General Main Grant Expenditure <ChevronsUpDown className="w-4 h-4 text-gray-600" /></div>
                                    </th>
                                    <th className="px-4 py-3 text-left text-sm font-semibold text-gray-700 tracking-wider whitespace-nowrap border-r border-white border-b border-gray-200">
                                        <div className="flex items-center justify-between">Capital Main Grant Allocation <ChevronsUpDown className="w-4 h-4 text-gray-600" /></div>
                                    </th>
                                    <th className="px-4 py-3 text-left text-sm font-semibold text-gray-700 tracking-wider whitespace-nowrap border-b border-gray-200">
                                        <div className="flex items-center justify-between">Capital Main Grant Expenditure <ChevronsUpDown className="w-4 h-4 text-gray-600" /></div>
                                    </th>
                                </tr>
                            </thead>
                                ...
                                {filteredItems.length > 0 ? (
                                    filteredItems.map((item, index) => (
                                        <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                                            ...
                                            <td className="px-4 py-3 text-sm text-gray-700 border-r border-gray-200">{item.salary_allocation}</td>
                                            <td className="px-4 py-3 text-sm text-gray-700 border-r border-gray-200">{item.salary_expenditure}</td>
                                            <td className="px-4 py-3 text-sm text-gray-700 border-r border-gray-200">{item.general_main_grant_allocation}</td>
                                            <td className="px-4 py-3 text-sm text-gray-700 border-r border-gray-200">{item.general_main_grant_expenditure}</td>
                                            <td className="px-4 py-3 text-sm text-gray-700 border-r border-gray-200">{item.capital_main_grant_allocation}</td>
                                            <td className="px-4 py-3 text-sm text-gray-700">{item.capital_main_grant_expenditure}</td>
                                        </tr>
                                    ))
                                ) : (
                                    <tr>
