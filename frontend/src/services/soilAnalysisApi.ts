...
const BASE = `${API_URL}/forms/soil-water/analysis`;

export const soilAnalysisApi = {
    getAll: async () => {
        const res = await fetch(BASE, fetchOptions);
        if (!res.ok) throw new Error('Failed to fetch soil analysis records');
        return res.json();
    },
    create: async (data: any) => {
        ...
        if (!res.ok) {
            ...
            throw new Error(err.message || 'Failed to create soil analysis record');
        }
        return res.json();
    },
