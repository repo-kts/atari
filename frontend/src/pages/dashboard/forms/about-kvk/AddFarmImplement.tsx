...
import { useKvkFarmImplements } from '../../../../hooks/useAboutKvkData'
...
export default function AddFarmImplement() {
    const navigate = useNavigate()
    const { user } = useAuthStore()
    const { create, isLoading } = useKvkFarmImplements()
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (!formData.implementName) {
                alert('Please enter Name of Implement')
                return
            }
            ...
            if (!formData.sourceOfFund) {
                alert('Please enter Source of fund')
                return
            }

            const payload = {
                kvkId: user.kvkId,
                implementName: formData.implementName,
                yearOfPurchase: parseInt(formData.yearOfPurchase),
                totalCost: parseFloat(formData.totalCost),
                presentStatus: formData.presentStatus as any, // String in UI, backend might have enum but existing code often uses string.
                sourceOfFund: formData.sourceOfFund
            }

            await create(payload as any)
            alert('Farm Implement added successfully')
            navigate(ENTITY_PATHS.KVK_FARM_IMPLEMENTS)
        } catch (error: any) {
            console.error('Error creating farm implement:', error)
            alert(error.message || 'Failed to add farm implement')
        }
    }
    ...
    return (
        <div className="bg-white rounded-2xl p-6 relative pb-20">
            ...
            <div className="sticky top-0 z-10 bg-white flex justify-between items-center mb-8 pb-4 border-b border-[#E0E0E0]">
                <h2 className="text-xl font-semibold text-[#2F3A4B]">Add Farm Implement</h2>
                <button
                    onClick={() => navigate(ENTITY_PATHS.KVK_FARM_IMPLEMENTS)}
            ...
            <h3 className="text-lg font-medium text-[#2F3A4B] mb-6">Implements Details</h3>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-x-8 gap-y-6">
                    ...
                    <div className="space-y-1.5 md:col-span-3">
                        <label className="text-sm font-medium text-[#374151]">
                            Name of Implement <span className="text-red-500">*</span>
                        </label>
                        <input
                            type="text"
                            value={formData.implementName}
                            onChange={(e) => handleChange('implementName', e.target.value)}
