...
export const TspScspForms: React.FC<TspScspFormsProps> = ({
    ...
    // appear on each page anyway, due to the client-side filter).
    useEffect(() => {
        if (!isSplitEntity) return
        if (formData?.id) return
        const expected = isScsp ? 'SCSP' : 'TSP'
        const current = (formData?.type || '').toString().toUpperCase()
        if (current !== expected) {
            setFormData({ ...formData, type: expected })
        }
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [entityType, formData?.id])
