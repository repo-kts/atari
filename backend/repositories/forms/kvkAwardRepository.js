...
const kvkAwardRepository = {
    create: async (data, user) => {
        ...
        const inserted = await prisma.$queryRawUnsafe(`
            ...
            ("kvkId", award_name, amount, achievement, conferring_authority, reporting_year, created_at, updated_at)
    ...
    update: async (id, data) => {
        ...
        if (updates.length > 0) {
            ...
            console.log('--- KVK Award Update ---');
            console.log('SQL:', sql);
            console.log('Values:', values);
            await prisma.$queryRawUnsafe(sql, ...values);
            console.log('Update executed successfully');
        }
        return await kvkAwardRepository.findById(id);
    },
    delete: async (id) => {
