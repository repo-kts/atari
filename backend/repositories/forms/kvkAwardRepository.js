...
const kvkAwardRepository = {
    create: async (data, user) => {
        try {
            ...
            const result = await prisma.kvkAward.create({
                data: createData,
                include: {
            ...
            throw new RepositoryError(`Failed to create KVK award record: ${error.message}`, 'CREATE_ERROR', 500);
        }
    },
    ...
    findAll: async (filters = {}, user) => {
        try {
            ...
            const records = await prisma.kvkAward.findMany({
                where,
                include: {
                    kvk: { select: { kvkName: true } },
                },
            ...
            throw new RepositoryError(`Failed to fetch KVK awards: ${error.message}`, 'FETCH_ERROR', 500);
        }
    },
    ...
    findById: async (id, user) => {
        try {
            ...
            const kvkAwardId = validateUUID(id, 'id', false);
            const where = { kvkAwardId };
            ...
            const record = await prisma.kvkAward.findFirst({
                where,
                include: {
                    kvk: { select: { kvkName: true } },
                },
            ...
            if (!record) {
                throw new RepositoryError('KVK award not found or unauthorized', 'NOT_FOUND', 404);
            }
            ...
            throw new RepositoryError(`Failed to fetch KVK award: ${error.message}`, 'FETCH_ERROR', 500);
        }
    },
    ...
    update: async (id, data, user) => {
        try {
            ...
            const existing = await prisma.kvkAward.findFirst({
                where,
                select: { kvkAwardId: true },
            });
            ...
            // Update the record
            const result = await prisma.kvkAward.update({
                where: { kvkAwardId },
                data: updateData,
                include: {
                    kvk: { select: { kvkName: true } },
            ...
            if (error instanceof Prisma.PrismaClientKnownRequestError) {
                if (error.code === 'P2025') {
                    throw new RepositoryError('KVK award not found', 'NOT_FOUND', 404);
                }
                if (error.code === 'P2003') {
                    throw new RepositoryError('Invalid foreign key reference', 'VALIDATION_ERROR', 400);
                }
            ...
            throw new RepositoryError(`Failed to update KVK award: ${error.message}`, 'UPDATE_ERROR', 500);
        }
    },
    ...
    delete: async (id, user) => {
        try {
            ...
            // Delete the record
            await prisma.kvkAward.delete({
                where: { kvkAwardId },
            });

            return { success: true, message: 'KVK award deleted successfully' };
        } catch (error) {
            if (error instanceof RepositoryError) {
                throw error;
            }
            ...
            throw new RepositoryError(`Failed to delete KVK award: ${error.message}`, 'DELETE_ERROR', 500);
        }
    },
};
