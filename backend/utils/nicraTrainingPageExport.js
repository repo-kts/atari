...
const { buildNicraTrainingGroups } = require('../services/reports/formsTemplate/projectTemplates/nicraTrainingTemplate.js');

const TITLE = 'NICRA — Training';
const TOTAL_COLS = 20; // S.No, Title, Period, Duration, Type, farmers(15)
...
function dataVals(r) {
    return [r.sl, r.title, r.period, r.duration, r.type, ...farmerCells(r)];
}
...
function writeGroupedHeader(ws, top) {
    ...
    ws.mergeCells(r1, 2, r2, 2); ws.getCell(r1, 2).value = 'Title of the training course';
    ws.mergeCells(r1, 3, r2, 3); ws.getCell(r1, 3).value = 'Period of Training program';
    ws.mergeCells(r1, 4, r2, 4); ws.getCell(r1, 4).value = 'Duration';
    ws.mergeCells(r1, 5, r2, 5); ws.getCell(r1, 5).value = 'Training Type';
...
function setColWidths(ws) {
    ...
    for (let c = 6; c <= TOTAL_COLS; c += 1) ws.getColumn(c).width = 5;
}

async function generateNicraTrainingExcelBuffer(reportTitle, rawData) {
    const { groups, isMultiKvk } = buildNicraTrainingGroups(rawData);
    const wb = new ExcelJS.Workbook();

    if (groups.length === 0) {
        const ws = wb.addWorksheet('NICRA Training');
...
function buildHeaderRows() {
    ...
    const row1 = new TableRow({
        ...
        children: [
            h('S.No.', { rowSpan: 2 }),
            h('Title of the training course', { rowSpan: 2, alignment: AlignmentType.LEFT }),
            h('Period of Training program', { rowSpan: 2 }),
            h('Duration', { rowSpan: 2 }),
            h('Training Type', { rowSpan: 2 }),
            h('General', { colSpan: 3 }),
            h('OBC', { colSpan: 3 }),
            h('SC', { colSpan: 3 }),
            h('ST', { colSpan: 3 }),
...
async function generateNicraTrainingWordBuffer(reportTitle, rawData) {
    const { groups } = buildNicraTrainingGroups(rawData);
