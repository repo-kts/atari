...
const { buildNfBeneficiariesGroups } = require('../services/reports/formsTemplate/projectTemplates/nfBeneficiariesTemplate.js');

const TITLE = 'Natural Farming — Details of Beneficiaries';
const FONT_HP = 16; // 8pt (matches the PDF body font)
const TOTAL_COLS = 9;
const HEADERS = [
    'S.No.', 'Reporting year', 'No. of blocks covered', 'No. of villages covered',
    'Total no. of Trained/Practicing NF Farmer', 'No. of farmers influenced to adopt NF',
    'No. of farmers engaged all season', 'No. of farmers engaged in 1 season', 'Remarks',
];
const LEFT_COLS = new Set([8]); // Remarks
...
function rowVals(r, idx) {
    return [
        idx + 1,
        r.reportingYear || '—',
        Number(r.blocksCovered || 0),
        Number(r.villagesCovered || 0),
        Number(r.totalTrainedFarmers || 0),
        Number(r.farmersInfluenced || 0),
        Number(r.farmersEngagedAllSeason || 0),
        Number(r.farmersEngagedOneSeason || 0),
        r.remarks || '—',
    ];
}
...
function writeGroupSheet(ws, g, reportTitle) {
    ...
    const widths = [6, 13, 12, 12, 18, 18, 16, 16, 26];
    widths.forEach((w, i) => { ws.getColumn(i + 1).width = w; });
}

async function generateNfBeneficiariesExcelBuffer(reportTitle, rawData) {
    const { groups, isMultiKvk } = buildNfBeneficiariesGroups(rawData);
    const wb = new ExcelJS.Workbook();

    if (groups.length === 0) {
        const ws = wb.addWorksheet('NF Beneficiaries');
...
async function generateNfBeneficiariesWordBuffer(reportTitle, rawData) {
    const { groups } = buildNfBeneficiariesGroups(rawData);
