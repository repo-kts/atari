...
const {
    ...
    fmtDate,
    fmtNum,
} = require('../services/reports/formsTemplate/projectTemplates/nfGeographicalTemplate.js');

const TITLE = 'Natural Farming — Geographical Information';
...
const LEFT_COLS = new Set([3, 4]); // Agro zone, Farming situation
...
function writeGroupSheet(ws, g, reportTitle) {
    ...
    const widths = [6, 14, 14, 28, 30, 12, 12];
    widths.forEach((w, i) => { ws.getColumn(i + 1).width = w; });
}

async function generateNfGeographicalExcelBuffer(reportTitle, rawData) {
    const { groups, isMultiKvk } = buildNfGeographicalGroups(rawData);
    const wb = new ExcelJS.Workbook();

    if (groups.length === 0) {
        const ws = wb.addWorksheet('NF Geographical');
...
async function generateNfGeographicalWordBuffer(reportTitle, rawData) {
    const { groups } = buildNfGeographicalGroups(rawData);
