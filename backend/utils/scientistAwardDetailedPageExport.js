...
const { buildScientistAwardGroups } = require('../services/reports/formsTemplate/achievementTemplates/scientistAwardDetailedTemplate.js');
...
const CENTERED = new Set([0, 3]); // Sl, Amount

function rowVals(r) {
    return [r.sl, r.scientist, r.award, r.amt, r.ach, r.auth];
}
...
async function generateScientistAwardDetailedExcelBuffer(reportTitle, rawData) {
    const { groups } = buildScientistAwardGroups(rawData);
    const wb = new ExcelJS.Workbook();
    const ws = wb.addWorksheet('Scientist Award');

    ws.mergeCells(1, 1, 1, COLS);
    const t1 = ws.getCell(1, 1);
    t1.value = reportTitle || 'Scientist Award';
...
async function generateScientistAwardDetailedWordBuffer(reportTitle, rawData) {
    const { groups } = buildScientistAwardGroups(rawData);

    const children = [
        new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: reportTitle || 'Scientist Award', bold: true, size: 18 })] }),
        new Paragraph({ text: '' }),
    ];
