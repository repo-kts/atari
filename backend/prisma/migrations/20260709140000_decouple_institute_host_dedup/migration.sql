...
-- ============================================================
-- 6. Repoint the denormalized org_id snapshot columns (targets,
--    module_images, user_login_activity) at the canonical orgMaster row.
--    These are plain scope-filter columns with no FK constraint, so the
--    deletes in step 8 would silently leave them dangling if skipped.
-- ============================================================
UPDATE "targets" t
SET "org_id" = c."canonical_id"
FROM "orgMaster" o
...
WHERE t."org_id" = o."org_id"
  AND t."org_id" <> c."canonical_id";

UPDATE "module_images" mi
SET "org_id" = c."canonical_id"
FROM "orgMaster" o
JOIN (
  SELECT "org_name", MIN("org_id") AS canonical_id
...
WHERE mi."org_id" = o."org_id"
  AND mi."org_id" <> c."canonical_id";

UPDATE "user_login_activity" ula
SET "org_id" = c."canonical_id"
FROM "orgMaster" o
JOIN (
  SELECT "org_name", MIN("org_id") AS canonical_id
...
WHERE ula."org_id" = o."org_id"
  AND ula."org_id" <> c."canonical_id";

-- ============================================================
-- 7. Delete the now-unreferenced duplicate orgMaster rows
--    (safe: every kvk/users/universityMaster/targets/module_images/
--    user_login_activity row that referenced a duplicate has already
--    been repointed to the canonical row above)
-- ============================================================
...
-- ============================================================
-- 8. Delete the now-unreferenced duplicate universityMaster rows
-- ============================================================
...
-- ============================================================
-- 9. Enforce the dedup going forward: one row per Institute/Host name
-- ============================================================
